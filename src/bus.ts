...
export default class Bus {
    ...
    private send(subscriptionId: string, data: any, type: MessageType = MessageType.Data) {
        if (!this.subscriptions[subscriptionId]) {
            this.logger.warn(`No subscription found matching '${subscriptionId}'`);
     ...
     */
    public processApiCall(id: string, callFn: CallFunction, params?: any) {
        ...
        return Promise.resolve(callFn(params))
            ...
            .catch((err: APIError) => {
                this.logger.error(`[${id.split('.')[0]}] ${id} - status code: ${err.status || err.statusCode}`);
    ...
    public unsubscribe(clientId: string, subscriptionId: string) {
        if (!this.clients[clientId]) {
            this.logger.warn(`unable to unsubscribe from '${subscriptionId}', client with id '${clientId}' does not exist`);
            return;
        }

        const subscription = this.subscriptions[subscriptionId];
        if (!subscription) {
            this.logger.warn(`unable to unsubscribe from '${subscriptionId}', subscription does not exist`);
            return;
        }
